  ...
  <div>
    ...
    <form class="ui form" :class="{ 'p-xl-x': Desktop && !inSubview, 'p-xs-x': inSubview }" @submit.prevent="Login">
      ...
      <div :class="{ 'p-sm-b': !inSubview }">
        <sui-login-submit :inSubview="!Desktop" v-model="user.remember" @update:showPass="showPass = $event" 
          recaptcha-action-name="login" @update:view="ChangeView(null)"/>
        <sui-login-links :inSubview="!Desktop" @update:view="ChangeView"/>
      </div>
    </form>
  </div>
</template>
...
import { LOGIN_POPUP_MODULE, GO_FORGOT_PASS_FORM, GO_LOGIN_FORM, GO_NEW_USER_FORM } from '../../Store/modules/LoginPopupState';
